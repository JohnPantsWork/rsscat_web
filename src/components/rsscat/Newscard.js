...
const Newscard = ({ card, setLikedTags, setDislikedTags }) => {
  ...
  return (
    <div className="newscard">
      <div className="operator">
        <img className="picture" src={card.picture ? card.picture : noPic} alt="empty" onClick={linkToNews} />
        <button onClick={like}>喜歡</button>
        <button onClick={dislike}>不喜歡</button>
      </div>
      <div className="details">
        <h4 className="title" onClick={linkToNews}>
          {card.title}
        </h4>
        <p className="auther">{card.auther}</p>
        <p className="des" onClick={linkToNews}>
          {card.des}
        </p>
      </div>
    </div>
